...
3.  **Deterministic Optimization Calculations:**
    ...
    *   *Use-Case-Specific Assistant Redundancy (Architectural Design Rationale):* The calculations engine enforces strict assistant seat consolidation rules (e.g., standardizing 80% of overlapping chat seats onto Claude when ChatGPT Plus is also active) **only** for `writing`, `research`, and `mixed` use cases. It deliberately bypasses this reduction for `coding` workflows. Architecturally, developer stacks consolidate cost at the **IDE/Editor layer** (e.g., standardizing on Cursor and removing overlapping GitHub Copilot or Windsurf), while browser assistants remain intact at full strength. This is because coding teams rely on highly distinct, non-overlapping specialized browser features—such as ChatGPT Plus's Python code execution sandbox, live document search, and custom API-builders, alongside Claude's raw logic reasoning context—and reducing either would trigger severe developer workflow bottlenecks.
